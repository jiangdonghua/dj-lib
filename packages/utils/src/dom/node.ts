...
export function getScrollParentNodes(node: HTMLElement): HTMLElement[] {
  ...
  const scrollParents = []
  let nodeName = getNodeName(parentNode)
...
export function hasRectContain(parentNode: HTMLElement, elm: HTMLElement): boolean {
  ...
  return (
    elmRect.top < parentRect.bottom &&
    elmRect.bottom > parentRect.top &&
    elmRect.right > parentRect.left &&
    elmRect.left < parentRect.right
  )
}
